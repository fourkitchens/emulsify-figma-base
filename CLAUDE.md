...
- `references/` — read-only canonical snapshot of a working `src/components/base/` plus Storybook config files (`preview-head.css`, `preview.js`, `storybook/main.js`). The skill instructs Claude to **read each reference file before writing its target counterpart** and to copy/structure/regenerate per tier.
...
- **Map-key rule.** `_spacing.scss` and `_typography.scss` map keys are **px values** (1, 2, 4, …, 120 / 12, 14, 16, …), not ordinals. Storybook chrome (`.sb-*` rules in the verbatim `preview-head.css`) consumes `var(--s-16)`, `var(--s-32)`, `var(--fs-14)`. Ordinal keys silently break the layout because SCSS-emitted vars win the cascade over preview-head's.
- **Colors as RGB triples in `preview-head.css`.** `clr()` wraps in `rgba(var(--clr-x), $alpha)`. Hex breaks every direct CSS consumer. Triples format: `--clr-link: 0, 95, 137;`.
- **Components must call Sass functions, never raw `var()`.** `clr()`, `space()`, `fontsize()`, `fontfamily()`, `fontweight()`, `lineheight()`, `letterspacing()`, and conditional `radius()`/`shadow()` are the only sanctioned read path. Two legal exceptions: `preview-head.css` (outside the Sass pipeline) and `*-variables.scss` entry points (the source of the vars).
- **`storybook/main.js` is required.** Without the legacy-API `sass-loader` patch, every component SCSS fails with `An importer must have either canonicalize and load methods…`. Emulsify-core's webpack uses `node-sass-glob-importer` (legacy); Dart Sass ≥ 1.45 defaults to modern.
- **`fonts/` layout depends on the font-loading strategy** (Step 3): local `@font-face` → entry `font.scss` + one partial per family; Google/Adobe → single `@import url(...)` in `font.scss` + `<link>` injected in `preview.js`; Drupal libraries → omit `fonts/` entirely.
- **`my_theme` strings in references = theme machine name, not brand.** They live in `preview.js` Twig namespaces, `my_theme:icon` includes, asset URL paths, and `drupalSettings` polyfill keys. CSS custom property prefixes (`--clr-`, `--s-`, etc.) and Sass map keys (`red:`, `grey:`) **never** carry a theme-name segment. Per `references/README.md`.
...
- **`preview.js` reference imports `./preview-head.js` and `../../../assets/fonts/sb-fonts.css`** — both are source-theme-specific. Delete the import line if the target has no such file; otherwise Storybook crashes at boot. Also strip `my_theme_search` / `my_theme_language` from the `drupalSettings` polyfill for any non-source theme.
